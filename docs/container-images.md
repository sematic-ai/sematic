...
is build natively, without using
...
- `push` is an optional section that describes how to push the image to a
  repository. A valida case to skip this section is if the Docker Server you
  are using is the same registry that is also used to source cloud images from.
  If this section is specified, then `push.registry` and `push.repository` must
  both be specified.
...
In the example below, this configuration value from `my_script.yaml` correctly
references the two data files: `data: ["../data1.txt", "//subdir/data2.txt"]`
...
5. If the resulting configuration has a `build` section, then:
   ...
   3. The `src` file globs are resolved and copied to the image, if any.
   4. The image is build for the specified `platform`(s), if any.
...
As stated, similarly to the Bazel Build System, custom base images can be
...
If a `build` section is not specified in the build configuration, then the base
image specified through `base_uri`, or obtained by executing the `image_script`
will be used as the final image fow workloads in the cloud, so they must
already contain all code and data dependencies.
