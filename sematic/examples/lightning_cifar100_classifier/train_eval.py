...
class CifarDataModule(pl.LightningDataModule):
    def __init__(self, batch_size, train_fraction, n_workers, base_class=torchvision.datasets.CIFAR10):
        super().__init__()
    ...
    def setup(self, stage: str):
        ...
        if stage == "fit":
            full = self.base_class("data", train=True, download=True, transform=self.train_transform)
            ...
            self.test = self.base_class("data", train=False, download=True, transform=self.test_transform)
            self.classes = self.test.classes
        elif stage == "predict":
            self.predict = self.base_class("data", train=False, download=True, transform=self.test_transform)
            self.classes = self.predict.classes
        else:
            raise ValueError(f"Unsupported stage '{stage}'")

    def train_dataloader(self):
        return DataLoader(self.train, num_workers=self.n_workers, batch_size=self.batch_size)

    def val_dataloader(self):
        return DataLoader(self.eval, num_workers=self.n_workers, batch_size=self.batch_size)

    def test_dataloader(self):
        return DataLoader(self.test, num_workers=self.n_workers, batch_size=self.batch_size)

    def predict_dataloader(self):
        return DataLoader(self.predict, num_workers=self.n_workers, batch_size=self.batch_size)
...
class LitResnet(LightningModule):
    ...
    def evaluate(self, batch, stage=None):
        ...
        if stage:
            ...
            if stage in ("fit", "val"):
                ...
                return
            self.log(f"{stage}_n_samples", float(len(preds)), reduce_fx="sum", sync_dist=True)
            count_by_confusion_key = Counter()
            for pred, label in zip(preds, y):
                count_by_confusion_key[(int(pred.detach().numpy()), int(label.detach().numpy()))] += 1
    ...
    def _create_confusion_metrics(self, stage, count_by_confusion_key):
        ...
        for (pred, label), count in count_by_confusion_key.items():
            metric_key_value_pairs.append((f"{stage}_{pred}_{label}_confusion", float(count)))
        return metric_key_value_pairs
    
    def _read_confusion_metrics(self, stage, metrics):
        confusion_dicts = [
            {"prediction": int(k.split("_")[1]), "label": int(k.split("_")[2]), "count": int(v)}
            for k, v in metrics[-1].items() if k.endswith("confusion") and k.startswith(stage)
        ]
        ...
        missing_predictions = set(range(self.num_classes)).difference(row["prediction"] for row in confusion_dicts)
        for missing_prediction in missing_predictions:
            # since this prediction didn't show up AT ALL, we know the count for
            # label 0 is 0.
            confusion_dicts.append({"prediction": missing_prediction, "label": 0, "count": 0})
        
        # Similar thing with missing labels: small evaluation data might not have them all,
        # but we want 0 example of each label so the table has every expected column.
        missing_labels = set(range(self.num_classes)).difference(row["label"] for row in confusion_dicts)
        ...
        return pd.DataFrame(confusion_dicts).pivot(index="prediction", columns="label", values="count").fillna(0)
...
def train_classifier(config: TrainingConfig, data_config: DataConfig, strategy_compute_kwargs: Dict[str, Union[float, int, bool]], checkpointer: SematicCheckpointIO) -> Checkpoint:
    seed_everything(42)
...
def evaluate_classifier(
    checkpoint: Checkpoint, config: EvaluationConfig, data_config: DataConfig, strategy_compute_kwargs: Dict[str, Union[float, int, bool]], checkpointer: SematicCheckpointIO,
) -> EvaluationResults:
    cifar_dm = CifarDataModule(
        batch_size=data_config.batch_size,
        train_fraction=data_config.train_fraction,
    ...
    model = LitResnet(batch_size=data_config.batch_size, num_classes=len(cifar_dm.classes), num_samples_per_epoch=-1)
