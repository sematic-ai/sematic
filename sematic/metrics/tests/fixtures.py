...
SeriesPoint = List[Union[float, List[str]]]
...
def check_approximate_equality(a: List[SeriesPoint], b: List[SeriesPoint]):
    ...
    for a_row, b_row in zip(a, b):
        ...
        for a_label, b_label in zip(a_labels, b_labels):
            assert isinstance(a_label, str), f"First series contained non-string label: {a_label}"
            assert isinstance(b_label, str), f"Second series contained non-string label: {b_label}"
