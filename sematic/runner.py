...
class Runner(abc.ABC):
    ...
    def run(self, future: AbstractFuture) -> typing.Any:
        """
        Abstract method. Entry-point for the execution algorithm.

        Parameters
        ----------
        future: AbstractFuture
            Root future of the graph to run.
    ...
    def entering_resource_context(cls, resource: AbstractExternalResource):
        """A hook runners may use to take action once a resource is activated.
    ...
    def exiting_resource_context(cls, resource_id: str):
        """A hook runners may use to take action once a resource is no longer used.
